...
int IWGRectangleButtonToTriangelBuffer(IWGRectangleButton * button, GLfloat* p)
{
    ...
    if (button->cornerCut & IWGRECTANGLEBUTTON_CORNER_CUT_LOWER_LEFT) {
        unsigned int indices[] = {2, 3, 0, 2, 0, 16, 16, 0, 15};
        ...
        unsigned int indices[] = {1, 3, 0, 1, 0, 15};
        for (int i = 0; i < sizeof(indices) / sizeof(unsigned int); i++) {
            p += IWGRectangleButtonBufferAppendVertex(p, baseVertices[indices[i]], -1.0, colour);
        }
    }
    if (button->cornerCut & IWGRECTANGLEBUTTON_CORNER_CUT_LOWER_RIGHT) {
        unsigned int indices[] = {3, 4, 0, 4, 6, 0, 6, 7, 0};
        for (int i = 0; i < sizeof(indices) / sizeof(unsigned int); i++) {
            p += IWGRectangleButtonBufferAppendVertex(p, baseVertices[indices[i]], -1.0, colour);
        }
    } else {
        unsigned int indices[] = {3, 5, 0, 5, 7, 0};
        for (int i = 0; i < sizeof(indices) / sizeof(unsigned int); i++) {
            p += IWGRectangleButtonBufferAppendVertex(p, baseVertices[indices[i]], -1.0, colour);
        }
    }
    if (button->cornerCut & IWGRECTANGLEBUTTON_CORNER_CUT_UPPER_RIGHT) {
        unsigned int indices[] = {0, 7, 8, 0, 8, 10, 0, 10, 11};
        for (int i = 0; i < sizeof(indices) / sizeof(unsigned int); i++) {
            p += IWGRectangleButtonBufferAppendVertex(p, baseVertices[indices[i]], -1.0, colour);
        }
    } else {
        unsigned int indices[] = {0, 7, 9, 0, 9, 11};
        for (int i = 0; i < sizeof(indices) / sizeof(unsigned int); i++) {
            p += IWGRectangleButtonBufferAppendVertex(p, baseVertices[indices[i]], -1.0, colour);
        }
    }
    if (button->cornerCut & IWGRECTANGLEBUTTON_CORNER_CUT_UPPER_LEFT) {
        unsigned int indices[] = {0, 11, 12, 0, 12, 14, 0, 14, 15};
        for (int i = 0; i < sizeof(indices) / sizeof(unsigned int); i++) {
            p += IWGRectangleButtonBufferAppendVertex(p, baseVertices[indices[i]], -1.0, colour);
        }
    } else {
        unsigned int indices[] = {0, 11, 13, 0, 13, 15};
        for (int i = 0; i < sizeof(indices) / sizeof(unsigned int); i++) {
            p += IWGRectangleButtonBufferAppendVertex(p, baseVertices[indices[i]], -1.0, colour);
        }
    }
